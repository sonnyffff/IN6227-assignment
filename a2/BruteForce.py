...
def run(filePath, start_row, end_row):
    # filePath = "bakery_sales_fixed_n.csv"
    nrows = end_row - start_row + 1
    start_time = time.time()

    transactions = pd.read_csv(filePath, header=None, delimiter=',', engine='python', names=range(100), skiprows=start_row, nrows=nrows)
    ...
    return time.time() - start_time, itemCnt